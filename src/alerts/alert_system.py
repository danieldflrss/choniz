...
class AlertSystem:
    ...
    async def send_system_alert(self, message: str, priority: AlertPriority = AlertPriority.MEDIUM, 
                               alert_type: AlertType = AlertType.SYSTEM, metadata: Dict = None):
    ...
    def _format_signal_alert(self, signal: Signal, priority: AlertPriority) -> str:
        ...
        if signal.signal_type == SignalType.CALL:
            direction_emoji = "🟢📈" if priority in [AlertPriority.HIGH, AlertPriority.CRITICAL] else "📈"
        else:
            direction_emoji = "🔴📉" if priority in [AlertPriority.HIGH, AlertPriority.CRITICAL] else "📉"
    ...
    def _is_duplicate_alert(self, message: str) -> bool:
        ...
        if message_hash in self.recent_alerts:
            ...
            if (current_time - last_time).total_seconds() < self.alert_config['duplicate_timeout']:
                return True
        ...
        cutoff_time = current_time - timedelta(seconds=self.alert_config['duplicate_timeout'] * 2)
        self.recent_alerts = {
    ...
    async def _dispatch_alert(self, alert_data: Dict):
        ...
        self.logger.info(f"{priority_symbol} ALERTA [{alert_data['priority']}]: \n{alert_data['message']}")
    ...
    def _analyze_performance_metrics(self, metrics: Dict) -> tuple[bool, AlertPriority, str]:
        ...
        if data_quality < 0.8:
            alert_needed = True
            priority = AlertPriority.HIGH if data_quality < 0.6 else AlertPriority.MEDIUM
        ...
        elif signals_per_hour < 1 and metrics.get('candles_processed', 0) > 60:  # Muy pocas señales
            alert_needed = True
            priority = AlertPriority.MEDIUM
    ...
    def get_recent_alerts(self, limit: int = 20, priority_filter: Optional[AlertPriority] = None,
                         type_filter: Optional[AlertType] = None) -> List[Dict]:
        ...
        # Aplicar filtros
        if priority_filter:
            filtered_alerts = [a for a in filtered_alerts if a['priority'] == priority_filter.value]
        
        if type_filter:
            filtered_alerts = [a for a in filtered_alerts if a['type'] == type_filter.value]
    ...
    def get_alert_statistics(self) -> Dict:
        ...
        stats.update({
            ...
            'priority_distribution_24h': self._get_priority_distribution(recent_24h),
    ...
    def create_custom_alert(self, message: str, priority: AlertPriority = AlertPriority.MEDIUM,
    ...
    def export_alert_history(self, format: str = 'json', limit: Optional[int] = None) -> str:
    ...
    def _export_to_csv(self, alerts: List[Dict]) -> str:
        ...
        # Headers CSV
        csv_lines = ["timestamp,type,priority,message,signal_type,confidence,entry_price"]
    ...
    def get_high_priority_summary(self) -> Dict:
        ...
        high_priority_alerts = [
            alert for alert in self.alert_history
            if alert.get('priority') in [AlertPriority.HIGH.value, AlertPriority.CRITICAL.value]
        ]
        ...
        recent_high_priority = [
            alert for alert in recent_high_priority
            if alert.get('priority') in [AlertPriority.HIGH.value, AlertPriority.CRITICAL.value]
        ]
        ...
        return {
            ...
            'latest_high_priority': high_priority_alerts[-1] if high_priority_alerts else None,
            'success_signals_high_priority': len([
                alert for alert in recent_high_priority
                if alert.get('type') == AlertType.SIGNAL.value and
                alert.get('metadata', {}).get('signal_quality') == 'Premium'
            ])
        }
    ...
    def __str__(self) -> str:
        ...
        return (f"AlertSystem(alertas_enviadas={self.alert_stats['total_sent']}, "
