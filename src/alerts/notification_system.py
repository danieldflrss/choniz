...
class NotificationSystem:
    ...
    def __init__(self):
        ...
        if self.enabled:
            ...
            logging.warning(
                "🔕 Sistema de notificaciones deshabilitado (plyer no disponible)"
            )
    ...
    def _check_rate_limits(self, alert_data: Dict) -> bool:
        ...
        self.notifications_this_minute = [
            t
            for t in self.notifications_this_minute
            if (current_time - t).total_seconds() < 60
        ]

        # Verificar límite por minuto
        if (
            len(self.notifications_this_minute)
            >= self.config["max_notifications_per_minute"]
        ):
        ...
        if alert_data.get("type") == "SIGNAL":
            ...
            if symbol in self.last_notification_time:
                time_diff = (
                    current_time - self.last_notification_time[symbol]
                ).total_seconds()
    ...
    def _process_signal_alert(self, alert_data: Dict):
        ...
        try:
            ...
            if signal_data:
                ...
                title, message, icon = self._prepare_signal_notification_content(
                    signal, priority
                )
            else:
                # Usar datos de metadata
                title, message, icon = self._prepare_signal_notification_from_metadata(
                    metadata, priority
                )
    ...
    def _process_error_alert(self, alert_data: Dict):
        ...
        try:
            ...
            self._send_notification(
                title, message, icon, alert_data, timeout=10
            )  # Errores duran más
    ...
    def _prepare_signal_notification_content(
        self, signal: Signal, priority: str
    ) -> tuple[str, str, str]:
    ...
    def _prepare_signal_notification_from_metadata(
        self, metadata: Dict, priority: str
    ) -> tuple[str, str, str]:
    ...
    def _send_notification_thread(
        self, title: str, message: str, icon: str, alert_data: Dict, timeout: int
    ):
        ...
        try:
            ...
            notification_params = {
                k: v for k, v in notification_params.items() if v is not None
            }
    ...
    def configure_notifications(self, **config):
        ...
        for key, value in config.items():
            if key in valid_configs:
                ...
                logging.info(
                    f"🔧 Configuración de notificaciones actualizada: {key} = {value}"
                )
            else:
    ...
    def get_notification_stats(self) -> dict:
        ...
        return {
            ...
            "last_notification": (
                self.notification_history[-1]["timestamp"]
                if self.notification_history
                else None
            ),
            "notifications_this_minute": len(self.notifications_this_minute),
            "rate_limit_active": len(self.notifications_this_minute)
            >= self.config["max_notifications_per_minute"],
        }
    ...
    def __str__(self) -> str:
        ...
        return (
            f"NotificationSystem(habilitado={self.enabled}, "
