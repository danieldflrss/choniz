...
class IEXCloudProvider:
    ...
    async def get_chart_data(self, symbol: str, range_period: str = '1d') -> List[Candle]:
            ...
            for bar in data:
                if all(k in bar for k in ['open', 'high', 'low', 'close']):
                    ...
                    if 'date' in bar and 'minute' in bar:
                        ...
                        timestamp = int(datetime.strptime(dt_str, '%Y-%m-%d %H:%M').timestamp())
                    elif 'date' in bar:
                        timestamp = int(datetime.strptime(bar['date'], '%Y-%m-%d').timestamp())
                    else:
