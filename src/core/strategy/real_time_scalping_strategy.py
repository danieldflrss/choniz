...
class RealTimeScalpingStrategy:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        # Usar configuración si se proporciona
        if config:
            strategy_config = config.get("strategy_config", {})
            self.min_signal_interval = strategy_config.get("min_signal_interval", 20)
            self.min_confidence = strategy_config.get("min_confidence", 0.65)
            self.volatility_threshold = strategy_config.get("volatility_threshold", 0.5)
            self.max_signals_per_hour = strategy_config.get("max_signals_per_hour", 15)

            # Configurar filtros desde config
            filters = config.get("filters", {})
            self.enable_trend_filter = filters.get("enable_trend_filter", True)
            self.enable_volatility_filter = filters.get(
                "enable_volatility_filter", True
            )
            self.enable_session_filter = filters.get("enable_session_filter", True)
            self.enable_confluence_filter = filters.get(
                "enable_confluence_filter", True
            )

            # Configurar indicadores desde config
            indicators_config = config.get("indicators_config", {})
            self.ema_periods = indicators_config.get("ema_periods", [9, 21, 50])
            self.rsi_periods = indicators_config.get("rsi_periods", [5, 14])
        else:
            # Valores por defecto
            self.min_signal_interval = 20
            self.min_confidence = 0.65
            self.volatility_threshold = 0.5
            self.max_signals_per_hour = 15
            self.enable_trend_filter = True
            self.enable_volatility_filter = True
            self.enable_session_filter = True
            self.enable_confluence_filter = True
    ...
    def _update_indicator_cache(self, candles: List[Candle]):
        ...
        # Medias móviles
        self._indicator_cache["ema9"] = self.technical_analysis.calculate_ema(
            closes, self.ema_periods[0]
        )
        self._indicator_cache["ema21"] = self.technical_analysis.calculate_ema(
            closes, self.ema_periods[1]
        )
        self._indicator_cache["ema50"] = self.technical_analysis.calculate_ema(
            closes, self.ema_periods[2]
        )
        self._indicator_cache["sma20"] = self.technical_analysis.calculate_sma(
            closes, 20
        )

        # Osciladores
        self._indicator_cache["rsi"] = self.technical_analysis.calculate_rsi(
            closes, self.rsi_periods[0]
        )
        self._indicator_cache["rsi_fast"] = self.technical_analysis.calculate_rsi(
            closes, self.rsi_periods[1]
        )
        ...
        bb_upper, bb_middle, bb_lower = (
            self.technical_analysis.calculate_bollinger_bands(closes)
        )
        ...
        self._indicator_cache["pa_patterns"] = (
            self.technical_analysis.detect_price_action_patterns(candles)
        )

        # Estructura del mercado
        self._indicator_cache["market_structure"] = (
            self.technical_analysis.calculate_market_structure(candles)
        )

        # Niveles clave
        self._indicator_cache["key_levels"] = (
            self.technical_analysis.find_dynamic_support_resistance(candles, 30, 2)
        )

        # Patrones de velas
        self._indicator_cache["candle_patterns"] = (
            self.technical_analysis.identify_advanced_patterns(candles[-10:])
        )

        self._cache_time = current_time

    def _price_action_momentum_strategy(
        ...
        if confidence >= 0.6 and signal_type != SignalType.WAIT:
            return self._create_signal(
                signal_type, confidence, current_candle.close, reasons
            )
    ...
    def _mean_reversion_strategy(self, candles: List[Candle]) -> Optional[Signal]:
        ...
        if (
            ...
            if (
                ...
                and current_candle.body_size > current_candle.total_range * 0.6
            ):
                confidence += 0.3
        ...
        if len(bb_upper) >= 5:
            ...
            bb_width_avg = (
                sum([(bb_upper[i] - bb_lower[i]) / bb_middle[i] for i in range(-5, -1)])
                / 4
            )
    ...
    def _breakout_strategy(self, candles: List[Candle]) -> Optional[Signal]:
        ...
        for level in key_levels:
            ...
            if (
                ...
                and distance_to_level < 0.001  # Ruptura reciente
                and current_candle.volume > 0
            ):
    ...
    def _confluence_strategy(self, candles: List[Candle]) -> Optional[Signal]:
        ...
        if confluence_score >= 0.6 and signal_type != SignalType.WAIT:
            return self._create_signal(
                signal_type, confluence_score, current_candle.close, reasons
            )
    ...
    def _create_signal(
        ...
        atr = self._indicator_cache.get("atr", [])
        # Usar configuración de expiración si está disponible
        if hasattr(self, "config") and "expiry_config" in self.config:
            expiry_config = self.config["expiry_config"]
            base_expiry = expiry_config.get("base_expiry", 60)

            # Ajustar por volatilidad si está habilitado
            if expiry_config.get("volatility_adjustment", False):
                market_structure = self._indicator_cache.get("market_structure", {})
                volatility = market_structure.get("volatility", 1.0)

                if volatility > 1.5:
                    expiry = max(expiry_config.get("min_expiry", 60), base_expiry - 30)
                elif volatility < 0.7:
                    expiry = min(expiry_config.get("max_expiry", 300), base_expiry + 60)
                else:
                    expiry = base_expiry
            else:
                expiry = base_expiry
        ...
        if atr:
            current_atr = atr[-1]
            stop_loss = (
                entry_price - (current_atr * 1.5)
                if signal_type == SignalType.CALL
                else entry_price + (current_atr * 1.5)
            )
            take_profit = (
                entry_price + (current_atr * 2)
                if signal_type == SignalType.CALL
                else entry_price - (current_atr * 2)
            )
        else:
            stop_loss = None
            take_profit = None
    ...
    def _find_nearest_level(
        self, price: float, levels: List[KeyLevel]
    ) -> Optional[float]:
    ...
    def _validate_signal_conditions(self, candles: List[Candle]) -> bool:
        ...
        if len(candles) >= 10:
            ...
            current_volatility = (
                candles[-1].total_range / avg_range if avg_range > 0 else 1
            )
