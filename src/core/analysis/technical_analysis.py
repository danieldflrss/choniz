...
class TechnicalAnalysis:
    ...
    def calculate_macd(
        ...
        if len(signal_line) > 0:
            ...
            histogram = [
                macd_adjusted[i] - signal_line[i] for i in range(len(signal_line))
            ]
    ...
    def _is_change_of_character(candles: List[Candle]) -> bool:
        ...
        return (early_trend > 0 and recent_trend < 0) or (
            early_trend < 0 and recent_trend > 0
        )
    ...
    def _is_liquidity_grab(candles: List[Candle]) -> bool:
        ...
        if (
            spike_candle.low < prev.low
            and current.close > spike_candle.open
            and spike_candle.lower_shadow > spike_candle.body_size
        ):
            return True

        # Liquidity grab bajista
        if (
            spike_candle.high > prev.high
            and current.close < spike_candle.open
            and spike_candle.upper_shadow > spike_candle.body_size
        ):
            return True
    ...
    def identify_advanced_patterns(
        ...
        for i in range(2, len(candles)):
            ...
            if (
                ...
                and current.body_size > previous.body_size * 1.5
            ):
    ...
    def _is_enhanced_pinbar(candle: Candle) -> bool:
        ...
        shadow_ratio = (
            max(candle.upper_shadow, candle.lower_shadow) / candle.total_range
        )
    ...
    def _is_enhanced_hammer(candle: Candle) -> bool:
        ...
        return (
            ...
            and candle.upper_shadow < candle.body_size * 0.3
            and candle.body_size > candle.total_range * 0.2
        )
    ...
    def _is_enhanced_shooting_star(candle: Candle) -> bool:
        ...
        return (
            ...
            and candle.lower_shadow < candle.body_size * 0.3
            and candle.body_size > candle.total_range * 0.2
        )
    ...
    def find_dynamic_support_resistance(
        candles: List[Candle], lookback: int = 20, min_touches: int = 2
    ) -> List[KeyLevel]:
        ...
        levels.extend(
            TechnicalAnalysis._process_levels(
                pivot_highs, tolerance, "resistance", min_touches
            )
        )
        ...
        levels.extend(
            TechnicalAnalysis._process_levels(
                pivot_lows, tolerance, "support", min_touches
            )
        )
    ...
    def _find_pivot_points(
        candles: List[Candle], type_point: str, window: int = 3
    ) -> List[float]:
    ...
    @staticmethod
    def _process_levels(
        prices: List[float], tolerance: float, level_type: str, min_touches: int
    ) -> List[KeyLevel]:
