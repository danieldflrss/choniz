...
class RealTimeMarketSystem:
    ...
    def __init__(self, data_source: str = "binance", **credentials):
        ...
        self.candles_buffer = []
        self.max_buffer_size = 500  # Aumentado para más historia
    ...
    async def _preload_historical_data(self, symbol: str, interval: str):
        ...
        try:
            if self.data_source == "iqoptions":
                ...
                # Obtener más historia para IQ Options
                historical_data = await self.data_provider.get_historical_data(symbol, 60, 400)
                if historical_data:
                    self.candles_buffer = historical_data[-self.max_buffer_size:]
                    self.logger.info(f"📊 Cargadas {len(self.candles_buffer)} velas históricas")
                
            elif self.data_source == "binance":
                await self.data_provider.connect()
                historical_data = await self.data_provider.get_historical_data(symbol, interval, 200)
                if historical_data:
                    self.candles_buffer = historical_data
                    self.logger.info(f"📊 Cargadas {len(self.candles_buffer)} velas de Binance")
    ...
    def _validate_historical_data(self):
        ...
        for i in range(1, len(self.candles_buffer)):
            time_diff = self.candles_buffer[i].timestamp - self.candles_buffer[i-1].timestamp
            if time_diff > 120:  # Gap mayor a 2 minutos
                gaps += 1
        
        # Verificar precios válidos
        invalid_prices = sum(1 for c in self.candles_buffer 
        ...
        self.logger.info(f"📈 Calidad de datos: {data_quality:.2%} | Gaps: {gaps} | Precios inválidos: {invalid_prices}")
    ...
    async def _start_forex_analysis(self, symbol: str):
        ...
        while self.is_running:
            try:
                current_rate = await self.data_provider.get_forex_data(from_symbol, to_symbol)
    ...
    async def _on_new_candle_enhanced(self, candle: Candle):
        ...
        try:
            ...
            if (self.candles_buffer and 
                self.candles_buffer[-1].timestamp >= candle.timestamp):
                return
    ...
    async def _validate_signal(self, signal: Signal) -> bool:
        ...
        if len(self.candles_buffer) >= 10:
            recent_volatility = statistics.stdev([c.close for c in self.candles_buffer[-10:]])
        ...
        if signal.support_resistance:
            distance = abs(signal.entry_price - signal.support_resistance) / signal.entry_price
        ...
        if len(self.signal_history) > 0:
            ...
            if len(self.signal_history) >= 10:
                for i in range(1, min(11, len(self.signal_history))):
                    prev_diff = (self.signal_history[-i].timestamp - 
                               self.signal_history[-i-1].timestamp).total_seconds()
        ...
        while self.is_running:
            try:
                ...
                if self.signal_count > 0:
                    ...
                    signal_frequency = self.signal_count / max(1, self.candle_count / 60)  # señales por hora
            ...
            try:
                ...
                if len(self.candles_buffer) >= 10:
                    ...
                    for i in range(1, min(50, len(self.candles_buffer))):
                        time_diff = (self.candles_buffer[-i].timestamp - 
                                   self.candles_buffer[-i-1].timestamp)
                        if time_diff > 120:  # Gap mayor a 2 minutos
                            gaps += 1
                    
                    if gaps > 5:
                        self.logger.warning(f"⚠️ Calidad de datos degradada: {gaps} gaps detectados")
                        
                        # Intentar reconexión si hay muchos gaps
                        if gaps > 15:
                            self.logger.info("🔄 Intentando reconexión por baja calidad de datos...")
    ...
    def _calculate_estimated_win_rate(self) -> float:
        ...
        high_confidence_signals = [s for s in self.signal_history[-50:] if s.confidence > 0.8]
        medium_confidence_signals = [s for s in self.signal_history[-50:] if 0.6 <= s.confidence <= 0.8]
        
        # Estimación basada en confianza (aproximada)
        estimated_win_rate = (
            len(high_confidence_signals) * 0.75 + 
            len(medium_confidence_signals) * 0.6
    ...
    def stop(self):
        ...
        if self.signal_count > 0:
            total_time = time.time() - (self.last_candle_timestamp - self.candle_count * 60)
    ...
    def get_real_time_status(self) -> Dict:
        ...
        status = {
            ...
            'estimated_win_rate': self._calculate_estimated_win_rate() if self.signal_count > 0 else None,
    ...
    def get_signal_statistics(self) -> Dict:
        ...
        return {
            ...
            'call_percentage': len(call_signals) / len(recent_signals) * 100 if recent_signals else 0,
    ...
    def configure_filters(self, trend_filter: bool = None, volatility_filter: bool = None, 
